        ...
        for (double c1 = 0; c1 < 2; c1 += incr)
        {
            for (double c2 = 0; c2 < 2; c2 += incr)
            {
                for (double c3 = 0; c3 < 2; c3 += incr)
                {
                    try
                    {
                        ProcessBuilder pb1 = new ProcessBuilder(String.format("java BatchPlayer 10 java Player asdf1234 %.2f %.2f %.2f", c1, c2, c3).split(" "));
                        Process p1 = pb1.start();

                        ProcessBuilder pb2 = new ProcessBuilder("java BatchPlayer 10 java StrictPlayer asdf1234".split(" "));
